...
| I-002 | FFI | P0 | Complete user-facing `extern "C"` blocks, C type mapping, `repr(C)` structs, variadic calls, `--link` support, `String::as_c_str()`, and end-to-end FFI tests. | `doc/FFI_DESIGN.md` says planned; `test/future_features/test_ffi_extern_c.vyn` is `@expect: fail`; source has partial `ExternStatement` support but no full block/variadic/repr(C) flow. |
