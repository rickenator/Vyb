...
def extract_test_case(test_case_text):
    ...
    original_filename = ""
    filename_match = re.search(r'(\w+\.vyb)', comment)
    if filename_match:
...
def save_test_cases(test_cases, output_dir):
    ...
    existing_files = set([f.name for f in Path(output_dir).glob("*.vyb")])

    for test in test_cases:
        # Check if file already exists
        if test["filename"] in existing_files:
            base_filename = test["filename"].replace(".vyb", "")
            count = 1
            while f"{base_filename}_{count}.vyb" in existing_files:
                count += 1
            test["filename"] = f"{base_filename}_{count}.vyb"
