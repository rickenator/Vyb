...
This document is the durable orientation guide for the Vyn project. It is meant
...
- Remote: `git@github.com:rickenator/Vyn.git`
...
Vyn is a statically typed systems programming language implemented in C++17 on
...
In compiler terms, Vyn is already a substantial language implementation: it has
...
- `include/vyn/`: public compiler/runtime headers
...
- `stdlib/`: early Vyn standard library modules
...
- 736 `.vyn` files under `test/`
- 13 top-level `.vyn` examples under `examples/`
- 5 `.vyn` demos under `demos/`
- 12 `.vyn` stdlib files under `stdlib/`

## 5. Language Surface

### 5.1 Name-First Syntax

Vyn functions put the function name first, then parameters, then return type:
...
- `doc/Vyn_Function_Declaration_Syntax.md`
...
Vyn's ownership model is a core language differentiator:
...
- `doc/Memory_Operations.md`
- `test/ffi/extern_c_requires_freedom.vyn`
- `test/new_features/test_freedom_block.vyn`
- `demos/ffi_freedom.vyn`
...
- `test/basic/simple_struct.vyn`
- `test/new_features/test_enum_basic.vyn`
- `test/new_features/test_enum_match.vyn`
- `src/vre/llvm/cgen_decl.cpp`

### 5.6 Aspects And Binds

Vyn's trait-like system uses `aspect` and `bind`.
...
- `test/template/`
- `test/new_features/test_generic_identity.vyn`
...
- `demos/control_flow.vyn`
- `test/new_features/test_defer_basic.vyn`
- `test/new_features/test_defer_lifo.vyn`
- `test/vec_for/`
- `test/range_for/`
...
- `doc/MATCH_SYNTAX.md`
- `test/select_match/`
- `test/new_features/test_select_expr.vyn`
- `test/new_features/test_match_basic.vyn`
...
- `test/vectors/`
- `test/vec_for/`
- `demos/collections.vyn`
- `src/vre/llvm/cgen_vec.cpp`

### 5.12 Tuples And Multi-Value Returns

Vyn supports tuple types and multi-value returns:
...
- `test/tuples/`
- `test/units/test_multi_value_return.vyn`
...
Vyn's error model uses failures and traps instead of ordinary exceptions.
...
- local path loading
- module search paths: importer directory, `--module-path`, `VYN_MODULE_PATH`,
  stdlib discovery
- `bundle(...)` visibility
...
- `doc/LAMBDAS.md`
- `test/lambda/`
- `test/future_features/test_lambda_codegen.vyn`
...
```bash
build/vyn program.vyn
build/vyn program.vyn --parse-only
build/vyn program.vyn --semantic-only
build/vyn program.vyn --emit-llvm
build/vyn program.vyn --compile program.o
build/vyn program.vyn --build program
build/vyn program.vyn --build program --link m
build/vyn program.vyn --module-path examples/modules
```
...
- Write five simple Vyn programs.
...
```bash
build/vyn file.vyn --parse-only
build/vyn file.vyn --semantic-only
build/vyn file.vyn --emit-llvm
build/vyn file.vyn
```
...
Vyn's ownership vocabulary is rich, but the full contract is not finished. The
...
- `doc/FEATURE_STATUS.md`
- this file: `doc/VYN_PROJECT_STUDY_GUIDE.md`
- `README.md`
- `TODO.md`
- `CHANGELOG.md`
- `UPDATE_LOG.md`
...
Mild reference:
Vyn's weak-reference ownership form.
...
ModuleRegistry:
Compiler subsystem that resolves and orders imported `.vyn` modules.

VRE:
Vyn Runtime Environment, used in docs for runtime and execution design.
