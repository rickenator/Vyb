...
| Forward references | ✅ | Within a module, name resolution is independent of implementation order — for FUNCTIONS (a function can call another function or itself declared later, and mutually-recursive functions resolve) and for TYPES (a struct/enum/type alias can be referenced before its declaration in struct fields, fn signatures/returns, construction, or member access; mutually-referential structs resolve either way). The semantic analyzer pre-registers every top-level function and type NAME in the module scope and analyzes type declarations before function bodies (mirroring codegen, which forward-declares functions and pre-declares opaque struct LLVM types, filling bodies/metadata afterward). Applies inside shared modules too (helpers must still be shared + imported, per the module contract). Duplicate definitions are still rejected (test/units/test_forward_references.vyb, test/units/test_forward_reference_duplicate.vyb, test/units/test_forward_ref_types.vyb, test/units/test_forward_ref_duplicate_type.vyb, test/modules/test_forward_ref_module.vyb + test/modules/fwdref/mod.vyb) |
| Structs | ✅ | |
...
| `tls`, `https` | Native `T?` + lossless reason | `tls_stream`/`tls_client_context` return `T?` (absent = failure); `https_get_full`/`https_get_full_verified` return a **present** `HttpResponse` whose `error<String?>` carries a lossless failure reason (`"resolve failed"`, `"connect ... failed"`, `"tls handshake failed"`, ...) with `status -1`, and whose body conveniences `https_get(h)/get_verified` return `String?` (absent when `status < 0`); `TlsError`/`HttpError` + `<mod>_error(op, target)` builders; the `https_selfhost*` diagnostics return `Bool?` (absent on any failure). |
