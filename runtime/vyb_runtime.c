...
// NULL used to truncate the probe early (linear probing can't stop at a deleted
// slot), so retain() could miss an entry that a later release() found — dropping
// the reference count and freeing a String the caller still held (the async
// http worker UAF). Probes keep walking past tombstones and only stop at a real
...
// holding `p` or a real NULL (end of the cluster) is reached. NULL means `p` is
...
// A Vec<String> element in generated code is the full `{ ptr, i64 }` String
// struct (16 bytes); the bulk helpers below stride by it to reach each element's
// data pointer. (Declared locally: the named runtime file-str type lives later.)
typedef struct { char* ptr; int64_t len; } vyb_str_elem;
...
// stored as a Vec element is the full `{ ptr, len }` struct, so a Vec<String>
// element buffer is a `{ ptr, len }*` array and these helpers stride by that
// struct (16 bytes) to reach each element's data pointer. Drop one reference on
...
VYB_WEAK void __vyb_string_release_each(void* arr, int64_t n) {
    ...
    for (int64_t i = 0; i < n; ++i) __vyb_string_release(e[i].ptr);
}
...
VYB_WEAK void __vyb_string_retain_each(void* arr, int64_t n) {
    if (!arr || n <= 0) return;
    vyb_str_elem* e = (vyb_str_elem*)arr;
    for (int64_t i = 0; i < n; ++i) __vyb_string_retain(e[i].ptr);
}
...
// #195 Item 1: recursively create `path` (all missing parent components), and be
// idempotent when a component already exists as a directory. Returns 0 on
// success, -1 on failure (other than EEXIST), with errno captured. Prefixed
// (`__vyb_`) so it never collides with libc `mkdir` when the runtime is
// co-linked into a standalone executable.
VYB_WEAK int64_t __vyb_mkdir(const char* path) {
    ...
    for (char* p = buf + 1; ; ++p) {
        if (*p == '/' || *p == '\0') {
            char saved = *p;
            *p = '\0';
            if (mkdir(buf, 0755) != 0 && errno != EEXIST) { rc = -1; }
            *p = saved;
            if (rc != 0 || saved == '\0') break;
        }
    }
    free(buf);
    return rc;
...
// #195 Item 2: SHA-256 (FIPS 180-4) digest. `data`/`len` is the message; the
// 32-byte big-endian digest is written to `out`. Compact reference impl.
static uint32_t vyb_ror32(uint32_t x, unsigned n) { return (x >> n) | (x << (32 - n)); }

static void vyb_sha256(const unsigned char* data, size_t len, unsigned char out[32]) {
    ...
    unsigned char* m = (unsigned char*)malloc(padded_len);
    if (!m) return;
    if (data && len) memcpy(m, data, len);
    m[len] = 0x80;
    for (size_t i = len + 1; i < padded_len - 8; ++i) m[i] = 0;
    for (int i = 0; i < 8; ++i) m[padded_len - 1 - i] = (unsigned char)(bitlen >> (8 * i));
    for (size_t blk = 0; blk < padded_len; blk += 64) {
        uint32_t w[64];
        for (int i = 0; i < 16; ++i)
            w[i] = ((uint32_t)m[blk + i*4] << 24) | ((uint32_t)m[blk+i*4+1] << 16)
                 | ((uint32_t)m[blk+i*4+2] << 8) | (uint32_t)m[blk+i*4+3];
        for (int i = 16; i < 64; ++i) {
            uint32_t s0 = vyb_ror32(w[i-15], 7) ^ vyb_ror32(w[i-15], 18) ^ (w[i-15] >> 3);
            uint32_t s1 = vyb_ror32(w[i-2], 17) ^ vyb_ror32(w[i-2], 19) ^ (w[i-2] >> 10);
            w[i] = w[i-16] + s0 + w[i-7] + s1;
        }
        uint32_t a=h[0],b=h[1],c=h[2],d=h[3],e=h[4],f=h[5],g=h[6],hh=h[7];
        for (int i = 0; i < 64; ++i) {
            uint32_t S1 = vyb_ror32(e,6) ^ vyb_ror32(e,11) ^ vyb_ror32(e,25);
            uint32_t ch = (e & f) ^ (~e & g);
            uint32_t t1 = hh + S1 + ch + K[i] + w[i];
            uint32_t S0 = vyb_ror32(a,2) ^ vyb_ror32(a,13) ^ vyb_ror32(a,22);
...
// SHA-256 of a byte buffer as a lowercase hex String. Returns { ptr, len=64 }
// with an owned registry-registered buffer (freed when the Vyb String built
// over it is released), or { NULL, 0 } on allocation failure.
VYB_WEAK vyb_file_str __vyb_sha256_hex(const char* data, int64_t len) {
    vyb_file_str r = { NULL, 0 };
    if (len < 0) { errno = EINVAL; return r; }
    unsigned char digest[32];
    vyb_sha256((const unsigned char*)(data ? data : ""), (size_t)len, digest);
    char* hexbuf = (char*)malloc(65);
    if (!hexbuf) { errno = ENOMEM; return r; }
    static const char* HX = "0123456789abcdef";
    for (int i = 0; i < 32; ++i) {
        hexbuf[i*2]   = HX[digest[i] >> 4];
        hexbuf[i*2+1] = HX[digest[i] & 0x0f];
    }
    hexbuf[64] = '\0';
    __vyb_string_register(hexbuf);
    r.ptr = hexbuf;
    r.len = 64;
    return r;
}
