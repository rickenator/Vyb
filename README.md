...
# Run the full test suite ( .vyb tests) with the canonical harness
...
### Test Features
- ** Tests, All Passing**: The full `run_tests.py` suite covers parse, semantic, modules, async, agents, tls, qt, and every other feature area
...
#### **Test Statistics**
- **Total Tests**:  `.vyb` tests (full suite, all passing as of v0.7.5)
...
- **Success Rate**: 100% (/) on the current suite
...
- ✅ **Modern Test Harness**: `test/run_tests.py` running the full suite —  `.vyb` tests all passing — with an auxiliary parallel/HTML/triage harness
