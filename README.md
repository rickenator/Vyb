...
# Run the full test suite (1163 .vyb tests) with the canonical harness
...
For multi-file projects, `vyb.toml` describes the package and every `[[bin]]`,
and `vyb build` compiles and links the whole project in one step:
...
### Test Features
- **1163 Tests, All Passing**: The full `run_tests.py` suite covers parse, semantic, modules, async, agents, tls, qt, and every other feature area
...
#### **Test Statistics**
- **Total Tests**: 1163 `.vyb` tests (full suite, all passing as of v0.7.5)
...
- **Success Rate**: 100% (1163/1163) on the current suite
...
- ✅ **Modern Test Harness**: `test/run_tests.py` running the full suite — 1163 `.vyb` tests all passing — with an auxiliary parallel/HTML/triage harness
