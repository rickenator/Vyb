...
- [x] **Test harness** — `--parse-only` flag forwarded to binary for `@parse-only: true` tests; `n/a` annotation values treated as "skip this check"; the canonical suite now runs **1184 tests, 1184 passing** via `test/run_tests.py --execute-jit` (re-anchored 2026-09-19)
