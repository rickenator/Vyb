...
Parser::Parser(const std::vector<vyb::token::Token>& tokens, std::string file_path)
    : tokens_(tokens),
      current_pos_(0),
...
std::unique_ptr<vyb::ast::Module> Parser::parse_module() { // Corrected return type
    ...
    if (!module_node) {
        ...
        while (temp_pos < this->tokens_.size() &&
               (this->tokens_[temp_pos].type == vyb::TokenType::COMMENT ||
                this->tokens_[temp_pos].type == vyb::TokenType::NEWLINE)) {
            temp_pos++;
        }
        if (temp_pos < this->tokens_.size() && this->tokens_[temp_pos].type != vyb::TokenType::END_OF_FILE) {
             throw std::runtime_error("Parser::parse_module: Failed to parse the entire module. Unexpected token at end: " + vyb::token_type_to_string(this->tokens_[temp_pos].type) + " in file " + this->file_path_);
        } else if (temp_pos >= this->tokens_.size() && !this->tokens_.empty() && this->tokens_.back().type != vyb::TokenType::END_OF_FILE) {
             throw std::runtime_error("Parser::parse_module: Token stream did not end with END_OF_FILE in file " + this->file_path_);
        }
        if (!module_node && temp_pos < this->tokens_.size() && this->tokens_[temp_pos].type == vyb::TokenType::END_OF_FILE) {
    ...
    while (final_check_pos < this->tokens_.size() &&
           (this->tokens_[final_check_pos].type == vyb::TokenType::COMMENT ||
            this->tokens_[final_check_pos].type == vyb::TokenType::NEWLINE)) {
        final_check_pos++;
    }

    if (final_check_pos < this->tokens_.size() && this->tokens_[final_check_pos].type != vyb::TokenType::END_OF_FILE) {
        throw std::runtime_error("Parser::parse_module: Did not consume all tokens. Remaining token: " + vyb::token_type_to_string(this->tokens_[final_check_pos].type) + " (\"" + this->tokens_[final_check_pos].lexeme + "\") at " + this->tokens_[final_check_pos].location.toString() + " in file " + this->file_path_);
    }
