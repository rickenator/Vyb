      ...
      m_currentVybModule(nullptr)
{
    ...
    // from the single authority instead of the (soon-removed) typeOfNode(node) field.
    // Falls back to typeOfNode(node) when no analyzer is registered (test/JIT paths);
