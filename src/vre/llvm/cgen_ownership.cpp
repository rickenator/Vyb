...
void LLVMCodegen::enterScope() {
    VDBG(std::cout << "DEBUG: Entering new scope (depth: " << scopeStack.size() + 1 << ")" << std::endl);
...
void LLVMCodegen::exitScope() {
    ...
    VDBG(std::cout << "DEBUG: Exiting scope (depth: " << scopeStack.size() << "), cleaning up "
...
void LLVMCodegen::registerVariable(const std::string& name, llvm::Value* allocaInst, llvm::Value* value, 
    ...
    VDBG(std::cout << "DEBUG: Registering variable '" << name << "' with ownership: "
    ...
    if (var.isVecWithMallocData) {
        VDBG(std::cout << "DEBUG: Variable '" << name << "' identified as Vec with malloc'd data" << std::endl);
    }
...
void LLVMCodegen::cleanupVariable(const ScopeVariable& var) {
    VDBG(std::cout << "DEBUG: Cleaning up variable '" << var.name << "', needsCleanup: "
              << var.needsCleanup << ", isVecWithMallocData: " << var.isVecWithMallocData << std::endl);
    
    // Skip cleanup if not needed
    if (!var.needsCleanup) {
        VDBG(std::cout << "DEBUG: Skipping cleanup for '" << var.name << "' (needsCleanup=false)" << std::endl);
        return;
    }
    ...
    switch (var.ownership) {
        case ast::OwnershipKind::MY: {
            ...
            if (var.isVecWithMallocData) {
                VDBG(std::cout << "DEBUG: Performing MY cleanup for Vec with malloc'd data: " << var.name << std::endl);
                ...
                VDBG(std::cout << "DEBUG: Generated free() call for " << var.name << std::endl);
                ...
                VDBG(std::cout << "DEBUG: Skipping MY cleanup for non-Vec variable: " << var.name << std::endl);
            }
            break;
        }
        ...
        case ast::OwnershipKind::OUR: {
            ...
            VDBG(std::cout << "DEBUG: Cleaning up OUR ownership for variable: " << var.name << std::endl);
        ...
        case ast::OwnershipKind::THEIR: {
            ...
            VDBG(std::cout << "DEBUG: No cleanup needed for borrowed reference: " << var.name << std::endl);
            break;
        }
        
        case ast::OwnershipKind::MILD: {
            // Weak reference - decrement weak_count in control block
            VDBG(std::cout << "DEBUG: Cleaning up MILD ownership for variable: " << var.name << std::endl);
...
void LLVMCodegen::incrementRefCount(const std::string& name) {
    VDBG(std::cout << "DEBUG: Incrementing refcount for: " << name << std::endl);
...
void LLVMCodegen::decrementRefCount(const std::string& name) {
    VDBG(std::cout << "DEBUG: Decrementing refcount for: " << name << std::endl);
    ...
    for (auto& scope : scopeStack) {
        for (auto& var : scope) {
            if (var.name == name && var.isVecWithMallocData) {
                VDBG(std::cout << "DEBUG: Performing OUR cleanup for Vec with malloc'd data: " << name << std::endl);
...
llvm::Function* LLVMCodegen::getOrCreateFreeFunction() {
    ...
    if (!freeFunc) {
        VDBG(std::cout << "DEBUG: Creating free() function declaration" << std::endl);
...
llvm::Function* LLVMCodegen::getOrCreateMallocFunction() {
    ...
    if (!mallocFunc) {
        VDBG(std::cout << "DEBUG: Creating malloc() function declaration" << std::endl);
...
llvm::Function* LLVMCodegen::getOrCreateMemsetFunction() {
    ...
    if (!memsetFunc) {
        VDBG(std::cout << "DEBUG: Creating memset() function declaration" << std::endl);
