...
void LLVMCodegen::generateTypeMetadata(const std::string& typeName, ast::StructDeclaration* structDecl) {
    ...
    VDBG(std::cout << "DEBUG: Generating type metadata for: " << typeName << std::endl);
    ...
    if (fieldMetadataArray.empty()) {
        VDBG(std::cout << "DEBUG: No fields to generate metadata for " << typeName << std::endl);
        return;
    }
    ...
    VDBG(std::cout << "DEBUG: Generated metadata for " << typeName
...
void LLVMCodegen::registerTypeMetadata() {
    if (typeMetadataGlobals.empty()) {
        VDBG(std::cout << "DEBUG: No type metadata to register" << std::endl);
        return;
    }
    
    VDBG(std::cout << "DEBUG: Registering " << typeMetadataGlobals.size() << " type metadata entries" << std::endl);
    ...
    for (const auto& pair : typeMetadataGlobals) {
        ...
        VDBG(std::cout << "DEBUG: Added registration call for type: " << pair.first << std::endl);
    }
    ...
    VDBG(std::cout << "DEBUG: Type metadata registration complete" << std::endl);
}
