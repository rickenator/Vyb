...
// CHECKPOINT B: map an AST node (possibly null) to its stable TypeTable key.
...
static unsigned exprKey(const vyb::ast::Node* n) { return n ? n->typeId() : 0; }
...
void SemanticAnalyzer::handleChanMethod(ast::CallExpression* node, ast::TypeNode* elem, const std::string& methodName) {
    if (methodName == "send") {
        ...
        setType(node,  retainType(intType));
        setType(node,  std::shared_ptr<ast::TypeNode>(intType->clone()));
        return;
    }
    if (methodName == "recv") {
        ...
        if (elemIsString) {
            ...
            setType(node,  retainType(elem->clone().release()));
            setType(node,  std::shared_ptr<ast::TypeNode>(elem->clone()));
        } else {
            ...
            setType(node,  retainType(optTy));
            setType(node,  std::shared_ptr<ast::TypeNode>(optTy->clone()));
        }
        return;
    }
    if (methodName == "poll") {
...
SemanticAnalyzer::~SemanticAnalyzer() {
    // Release synthesized type nodes created during analysis. These are
    ...
    // (or an imbalance on the error path) is still reclaimed.
    while (currentScope) {
...
static bool injectBareEnumConstructorType(SemanticAnalyzer& self, ast::Expression* expr, ast::TypeNode* targetType) {
    ...
    self.setType(expr,  std::shared_ptr<ast::TypeNode>(targetType->clone()));
    return true;
}
...
void SemanticAnalyzer::injectBareEnumCtorArgTypes(ast::CallExpression* node) {
    ...
    auto tryInject = [&](ast::Expression* arg, ast::TypeNode* paramType) {
        ...
        injectBareEnumConstructorType(*this, arg, paramType);
    };
...
void SemanticAnalyzer::visit(ast::Identifier* node) {
    ...
    setType(node,  readType ? std::shared_ptr<ast::TypeNode>(readType->clone()) : nullptr);
    ...
    if (readType) {
        setType(node,  std::shared_ptr<ast::TypeNode>(readType->clone()));
...
void SemanticAnalyzer::visit(ast::IntegerLiteral* node) {
    ...
    setType(node,  retainType(type));
    setType(node,  std::shared_ptr<ast::TypeNode>(type->clone()));
}
...
void SemanticAnalyzer::visit(ast::FunctionDeclaration* node) {
    ...
    for (auto& param : node->params) {
        if (param.name) {
            ...
            if (param.typeNode) {
                ...
                ast::TypeNode* effectiveType = typeOf(resolvedType) ? typeOf(resolvedType).get() : resolvedType;
    ...
    if (node->returnTypeNode) {
        ...
        ast::TypeNode* effectiveReturnType = typeOf(node->returnTypeNode) ? typeOf(node->returnTypeNode).get() : node->returnTypeNode.get();
    ...
    if (!processingTraitOrBindMethod) {
        ...
        if (funcSymFromTable) {
            ...
            if (funcSymFromTable->type) {
                 setType(node,  std::shared_ptr<ast::TypeNode>(funcSymFromTable->type->clone().release()));
            }
        }
    } else {
        // For trait/bind methods, still set the node's type directly
        setType(node,  std::shared_ptr<ast::TypeNode>(new ast::FunctionType(node->loc, std::move(paramTypesVec), std::unique_ptr<ast::TypeNode>(returnTypeAstNode))));
    }
...
void SemanticAnalyzer::visit(ast::VariableDeclaration* node) {
    ...
    if (node->init) {
            ...
            if (isVecCtor && node->typeNode) {
                ...
                if (auto vecVarType = dynamic_cast<ast::VecType*>(varType)) {
                    if (vecVarType->elementType) {
                        ...
                        setType(callExpr,  vecType ? std::shared_ptr<ast::TypeNode>(vecType->clone()) : nullptr);
                        setType(callExpr,  std::shared_ptr<ast::TypeNode>(std::move(vecType)));
                    }
                }
            }
        }
        ...
        if (needsTypeCheck && expressionTypes.count(exprKey(node->init.get()))) {
            ast::TypeNode* initType = expressionTypes[exprKey(node->init.get())].get();
            if (initType) {
        ...
        if (node->typeNode && expressionTypes.count(exprKey(node->init.get()))) {
        ...
        if (node->typeNode && node->init) {
            if (auto* initIdent = dynamic_cast<ast::Identifier*>(node->init.get())) {
                ...
                ast::TypeNode* destType = typeOf(node->typeNode)
                    ? typeOf(node->typeNode).get() : node->typeNode.get();
    ...
    // Get the resolved type - prefer the resolved type from typeOf(typeNode) if available
    ast::TypeNode* symbolType = nullptr;
    if (node->typeNode) {
        // If the typeNode has been resolved (e.g., typeOf(TypeName) contains the resolved VecType),
        // use that instead of the TypeName itself
        if (typeOf(node->typeNode)) {
            symbolType = typeOf(node->typeNode).get();
        } else {
            symbolType = node->typeNode.get();
        }
    } else if (node->init && expressionTypes.count(exprKey(node->init.get()))) {
        // For type inference without explicit type annotation
        symbolType = expressionTypes[exprKey(node->init.get())].get();
    }
...
void SemanticAnalyzer::visit(ast::TypeAliasDeclaration* node) {
    ...
    if (node->name && node->typeNode && typeOf(node->typeNode)) {
        currentScope->add(SymbolInfo{SymbolInfo::Kind::Type, node->name->name, false, ast::OwnershipKind::MY, retainType(typeOf(node->typeNode)->clone().release())}); // Explicit SymbolInfo
    } else if (node->name) {
...
void SemanticAnalyzer::visit(ast::UnaryExpression* node) {
    ...
    if (expressionTypes.count(exprKey(node->operand.get()))) {
        setType(node,  expressionTypes[exprKey(node->operand.get())]);
    } else if (typeOf(node->operand)) {
        setType(node,  typeOf(node->operand) ? std::shared_ptr<ast::TypeNode>(typeOf(node->operand)->clone()) : nullptr);
        setType(node,  std::shared_ptr<ast::TypeNode>(typeOf(node->operand)->clone()));
    }
}
void SemanticAnalyzer::visit(ast::BinaryExpression* node) {
    ...
    auto leftTypeIt = expressionTypes.find(exprKey(node->left.get()));
    auto rightTypeIt = expressionTypes.find(exprKey(node->right.get()));
    ...
    if (resultType) {
        setType(node,  resultType ? std::shared_ptr<ast::TypeNode>(resultType->clone()) : nullptr);
        setType(node,  std::shared_ptr<ast::TypeNode>(resultType->clone()));
...
void SemanticAnalyzer::visit(ast::CallExpression* node) {
    ...
    if (auto calleeId = dynamic_cast<ast::Identifier*>(node->callee.get())) {
        ...
        if (typeOf(node) && (cn == "Ok" || cn == "Err")) {
            auto* etn = dynamic_cast<ast::TypeName*>(typeOf(node).get());
    ...
    if (auto ident = dynamic_cast<ast::Identifier*>(node->callee.get())) {
        ...
        if (name == "lit" || name == "notype" || name == "bare" || name == "deserial" ||
            ...
            name == "vyb_cond_signal" || name == "vyb_cond_broadcast" ||
            name == "vyb_cond_free" ||
        ...
        // gen_qt[sem_allow]: begin
            ...
            name == "vyb_qt_web_zoom_in" || name == "vyb_qt_web_zoom_out" ||
            ...
            name == "vyb_qt_screen_width" || name == "vyb_qt_screen_height" ||
            name == "vyb_qt_screen_dpi" || name == "vyb_qt_label_create" ||
            name == "vyb_qt_label_set_text" || name == "vyb_qt_label_text" ||
            ...
            name == "vyb_qt_checkbox_set_checked" || name == "vyb_qt_progress_create" ||
            name == "vyb_qt_progress_set_value" || name == "vyb_qt_vbox" ||
            name == "vyb_qt_hbox" || name == "vyb_qt_layout_add" ||
            name == "vyb_qt_layout_add_layout" || name == "vyb_qt_layout_set_stretch" ||
            name == "vyb_qt_kind" || name == "vyb_qt_event_count" ||
            name == "vyb_qt_event_handle" || name == "vyb_qt_event_kind" ||
            name == "vyb_qt_event_pop" || name == "vyb_qt_wait_event" ||
            name == "vyb_qt_run" || name == "vyb_qt_run_stop" ||
            name == "vyb_qt_on_event" || name == "vyb_qt_post_event" ||
            ...
            name == "vyb_qt_widget_set_visible" || name == "vyb_qt_widget_visible" ||
            name == "vyb_qt_tabs_create" || name == "vyb_qt_tabs_add" ||
            name == "vyb_qt_tabs_count" || name == "vyb_qt_tabs_current" ||
            name == "vyb_qt_tabs_set_current" || name == "vyb_qt_list_create" ||
            name == "vyb_qt_list_add" || name == "vyb_qt_list_count" ||
            name == "vyb_qt_list_current" || name == "vyb_qt_list_set_current" ||
            name == "vyb_qt_list_item_text" || name == "vyb_qt_main_window_create" ||
            name == "vyb_qt_menubar" || name == "vyb_qt_menu_add" ||
            name == "vyb_qt_action_add" || name == "vyb_qt_action_count" ||
            name == "vyb_qt_statusbar_message" || name == "vyb_qt_statusbar_text" ||
            name == "vyb_qt_toolbar_create" || name == "vyb_qt_msg_info" ||
            name == "vyb_qt_msg_warn" || name == "vyb_qt_msg_error" ||
            name == "vyb_qt_msg_about" || name == "vyb_qt_msg_question" ||
            name == "vyb_qt_file_open" || name == "vyb_qt_file_save" ||
            name == "vyb_qt_dir_select" || name == "vyb_qt_dlg_info" ||
            name == "vyb_qt_dlg_warn" || name == "vyb_qt_dlg_error" ||
            name == "vyb_qt_dlg_about" || name == "vyb_qt_dlg_question" ||
            name == "vyb_qt_dlg_open" || name == "vyb_qt_dlg_save" ||
            name == "vyb_qt_dlg_dir" || name == "vyb_qt_dlg_close" ||
            name == "vyb_qt_dlg_selected" || name == "vyb_qt_event_result" ||
...
// gen_qt[sem_allow]: end
            ...
            setType(node,  std::make_unique<ast::TypeName>(
                ...
                if (auto varId = dynamic_cast<ast::Identifier*>(memCtor->property.get())) {
                    if (enumGenericParamOrder.count(baseId->name)) {
                        ...
                        if (enumIt != enumVariantPayloadTypes.end()) {
                            ...
                            setType(node,  std::shared_ptr<ast::TypeNode>(enumType.release()));
                            setType(node,  typeOf(node) ? std::shared_ptr<ast::TypeNode>(typeOf(node)->clone()) : nullptr);
    ...
    if (auto ident = dynamic_cast<ast::Identifier*>(node->callee.get())) {
        ...
        if (name == "lit" || name == "notype" || name == "bare" || name == "deserial") {
            ...
            if (name == "notype") {
                ...
                if (argExpr) {
                    argType = typeOf(argExpr).get();
                    if (!argType) {
                        auto it = expressionTypes.find(exprKey(argExpr));
            ...
            if (argExpr) {
                ...
                if (typeOf(argExpr)) {
                    setType(node,  std::shared_ptr<ast::TypeNode>(typeOf(argExpr)->clone().release()));
                    setType(node,  typeOf(argExpr) ? std::shared_ptr<ast::TypeNode>(typeOf(argExpr)->clone()) : nullptr);
                } else if (auto argType = expressionTypes[exprKey(argExpr)]) {
                    setType(node,  std::shared_ptr<ast::TypeNode>(argType->clone().release()));
                    setType(node,  argType);
                } else {
                    ...
                    setType(node,  std::shared_ptr<ast::TypeNode>(stringType));
                    setType(node,  stringType ? std::shared_ptr<ast::TypeNode>(stringType->clone()) : nullptr);
                }
            }
            return;
        }
        ...
        if (name == "borrow" || name == "view") {
            ...
            auto argTypeIt = expressionTypes.find(exprKey(argExpr));
            ...
            setType(node,  retainType(resultType));
            setType(node,  std::shared_ptr<ast::TypeNode>(resultType->clone().release()));
            return;
        }
        ...
        if (name == "my" || name == "their" || name == "our") {
            ...
            if (typeOf(argExpr)) {
                ...
                auto typeIt = expressionTypes.find(exprKey(argExpr));
        ...
        if (name == "println") {
            ...
            setType(node,  retainType(voidType));
            setType(node,  std::shared_ptr<ast::TypeNode>(voidType->clone().release()));
            return;
        }
        ...
        // Handle math library intrinsics
        {
            ...
            if (floatMathFuncs.count(name)) {
                ...
                setType(node,  retainType(floatType));
                setType(node,  std::shared_ptr<ast::TypeNode>(floatType->clone()));
                return;
            }
            if (intOrFloatFuncs.count(name)) {
                ...
                auto firstArgTypeIt = (node->arguments.size() > 0) ? expressionTypes.find(exprKey(node->arguments[0].get())) : expressionTypes.end();
                if (firstArgTypeIt != expressionTypes.end() && firstArgTypeIt->second) {
                    setType(node,  firstArgTypeIt->second);
                    setType(node,  std::shared_ptr<ast::TypeNode>(firstArgTypeIt->second->clone()));
                } else {
                    auto intType = new ast::TypeName(node->loc, std::make_unique<ast::Identifier>(node->loc, "Int"));
                    setType(node,  retainType(intType));
                    setType(node,  std::shared_ptr<ast::TypeNode>(intType->clone()));
        ...
        if (name == "agent_start" || name == "agent_start_bool" ||
            ...
            setType(node,  retainType(resTy));
            setType(node,  std::shared_ptr<ast::TypeNode>(resTy->clone()));
            return;
        }
...
// gen_qt[sem_str]: end
        ...
        auto resolveGenericCallReturnType =
            ...
            if (!node->explicitTypeArgs.empty()) {
                ...
                for (size_t i = 0; i < node->arguments.size() && i < funcDecl->params.size(); ++i) {
                    auto argTypeIt = expressionTypes.find(exprKey(node->arguments[i].get()));
                    ast::TypeNode* argType = (argTypeIt != expressionTypes.end()) ? argTypeIt->second.get() : nullptr;
                    if (argType) {
            ...
            if (auto functionType = dynamic_cast<ast::FunctionType*>(functionSymbol->type.get())) {
                if (functionType->returnType) {
                    ...
                    if (isGenericCall) {
                        ...
                        if (substituted) {
                            setType(node,  std::shared_ptr<ast::TypeNode>(substituted.release()));
                            setType(node,  typeOf(node) ? std::shared_ptr<ast::TypeNode>(typeOf(node)->clone()) : nullptr);
                        }
                    } else {
                        ast::TypeNode* returnType = typeOf(functionType->returnType)
                            ? typeOf(functionType->returnType).get()
                            : functionType->returnType.get();
                        setType(node,  returnType ? std::shared_ptr<ast::TypeNode>(returnType->clone()) : nullptr);
                        setType(node,  std::shared_ptr<ast::TypeNode>(returnType->clone()));
                    }
                }
        ...
        if (registryIt != functionRegistry.end() && registryIt->second && registryIt->second->returnTypeNode) {
            ...
            if (!typeOf(declaredReturn)) {
                declaredReturn->accept(*this);
            }
            ast::TypeNode* returnType = typeOf(declaredReturn) ? typeOf(declaredReturn).get() : declaredReturn;
            setType(node,  returnType ? std::shared_ptr<ast::TypeNode>(returnType->clone()) : nullptr);
            setType(node,  std::shared_ptr<ast::TypeNode>(returnType->clone()));
    ...
    if (auto memberExpr = dynamic_cast<ast::MemberExpression*>(node->callee.get())) {
            ...
            if (auto newIdent = dynamic_cast<ast::Identifier*>(memberExpr->property.get())) {
                if (vecIdent->name == "Vec" && newIdent->name == "new") {
                    ...
                    if (expressionTypes.count(exprKey(node)) && expressionTypes[exprKey(node)]) {
                        ...
                        VYB_CDBG << "DEBUG: Vec::new() type already propagated: " << expressionTypes[exprKey(node)]->toString() << std::endl;
                        return;
                    }
                ...
                if (methodName == "from_string") {
                    ...
                    auto argTypeIt = expressionTypes.find(exprKey(node->arguments[0].get()));
                ...
                if (typeName == "String" && methodName == "from_bytes") {
                    ...
                    setType(node,  retainType(strType));
                    setType(node,  std::shared_ptr<ast::TypeNode>(strType->clone()));
                    return;
                }
                ...
                if (typeName == "String" && methodName == "from_byte") {
                    ...
                    setType(node,  retainType(strType2));
                    setType(node,  std::shared_ptr<ast::TypeNode>(strType2->clone()));
                    return;
                }
            }
        }
                    ...
                    auto resolveAspectMethodForTypeString = [&](const std::string& typeNameStr) -> bool {
                        ...
                        if (candidates[0].returnType) {
                            ...
                            if (candidates[0].synthesized) {
                                ...
                                setType(node,  retainType(stableCopy));
                                setType(node,  std::shared_ptr<ast::TypeNode>(stableCopy->clone()));
                           } else {
                                setType(node,  actualReturnType ? std::shared_ptr<ast::TypeNode>(actualReturnType->clone()) : nullptr);
                            ...
                            if (typeNameStr.find("mild<") == 0) {
                                if (methodName == "grab") {
                                    ...
                                    auto boolType = new ast::TypeName(node->loc,
                                        std::make_unique<ast::Identifier>(node->loc, "Bool"));
                                    setType(node,  retainType(boolType));
                                    setType(node,  std::shared_ptr<ast::TypeNode>(boolType->clone()));
                                    return;
                                }
                            }
                        ...
                        if (objTypeName->identifier) {
                            ...
                            if (methodName == "to_string" &&
                                ...
                                auto stringType = new ast::TypeName(node->loc,
                                    std::make_unique<ast::Identifier>(node->loc, "String"));
                                setType(node,  retainType(stringType));
                                setType(node,  std::shared_ptr<ast::TypeNode>(stringType->clone()));
                    ...
                    if (auto objTypeName = dynamic_cast<ast::TypeName*>(objSymbol->type.get())) {
                        if (objTypeName->identifier && objTypeName->identifier->name == "String") {
                            ...
                            if (methodName == "len" || methodName == "length") {
                                ...
                                setType(node,  retainType(intPtrType));
                                setType(node,  std::shared_ptr<ast::TypeNode>(intPtrType->clone()));
                                return;
                            }
                        }
                    }
        ...
        if (auto methodIdent = dynamic_cast<ast::Identifier*>(memberExpr->property.get())) {
            ...
            auto objTypeIt = expressionTypes.find(exprKey(memberExpr->object.get()));
                                ...
                                for (ast::FunctionDeclaration* method : methods) {
                                    if (method && method->id && method->id->name == methodName) {
                                        ...
                                        if (method->returnTypeNode) {
                                            ...
                                            setType(node,  std::shared_ptr<ast::TypeNode>(method->returnTypeNode->clone()));
                                        }
    ...
    if (auto ident = dynamic_cast<ast::Identifier*>(node->callee.get())) {
        ...
        if (it != functionRegistry.end() && it->second->returnTypeNode) {
            setType(node,  it->second->returnTypeNode ? std::shared_ptr<ast::TypeNode>(it->second->returnTypeNode->clone()) : nullptr);
            setType(node,  std::shared_ptr<ast::TypeNode>(it->second->returnTypeNode->clone()));
        }
    }
}
...
void SemanticAnalyzer::visit(ast::ArrayElementExpression* node) {
    ...
    if (arrayTypeIt != expressionTypes.end() && arrayTypeIt->second) {
        if (auto arrayType = dynamic_cast<ast::ArrayType*>(arrayTypeIt->second.get())) {
            ...
            if (arrayType->elementType) {
                setType(node,  retainType(arrayType->elementType->clone().release()));
                setType(node,  std::shared_ptr<ast::TypeNode>(arrayType->elementType->clone()));
            }
        }
    }
    ...
    if (expressionTypes.find(exprKey(node)) == expressionTypes.end() &&
        ...
        if (auto typeName = dynamic_cast<ast::TypeName*>(resolvedType)) {
            if (typeOf(typeName)) resolvedType = typeOf(typeName).get();
        }
        if (auto tupleType = dynamic_cast<ast::TupleTypeNode*>(resolvedType)) {
            ...
            if (hasIndex) {
                ...
                setType(node,  retainType(member->clone().release()));
                setType(node,  std::shared_ptr<ast::TypeNode>(member->clone()));
    ...
    auto indexTypeIt = expressionTypes.find(exprKey(node->index.get()));
...
void SemanticAnalyzer::visit(ast::MemberExpression* node) {
    ...
    if (auto gi = dynamic_cast<ast::GenericInstantiationExpression*>(node->object.get())) {
        ...
        if (baseId && enumGenericParamOrder.count(baseId->name)) {
            auto typeIt = expressionTypes.find(exprKey(gi));
            ...
            if (enumPayIt != enumVariantPayloadTypes.end()) {
                ...
                auto enumType = std::make_unique<ast::TypeName>(node->loc,
                    std::make_unique<ast::Identifier>(node->loc, baseId->name));
                for (auto& a : gi->genericArguments) enumType->genericArgs.push_back(a->clone());
                setType(node,  std::shared_ptr<ast::TypeNode>(enumType->clone()));
                setType(node,  retainType(enumType.release()));
                return;
            }
        }
    }
        ...
        if (enumTypeNames.count(typeName)) {
            ...
            if (constEnumValues.count(typeName + "::" + methodName)) {
                auto* intTy = new ast::TypeName(node->loc,
                    std::make_unique<ast::Identifier>(node->loc, "Int"));
                setType(node,  std::shared_ptr<ast::TypeNode>(intTy->clone()));
                setType(node,  retainType(intTy));
                return;
            }
            ...
            if (enumPayIt != enumVariantPayloadTypes.end()) {
                ...
                auto* enumVariantTy = new ast::TypeName(node->loc,
                    std::make_unique<ast::Identifier>(node->loc, typeName));
                setType(node,  std::shared_ptr<ast::TypeNode>(enumVariantTy->clone()));
                setType(node,  retainType(enumVariantTy));
                return;
            }
    ...
    auto it = expressionTypes.find(exprKey(node->object.get()));
    if (it == expressionTypes.end() || !it->second) {
    ...
    if (fieldName == "tag" &&
        ...
        auto* tagTy = new ast::TypeName(node->loc,
            std::make_unique<ast::Identifier>(node->loc, "Int"));
        setType(node,  retainType(tagTy));
        setType(node,  std::shared_ptr<ast::TypeNode>(tagTy->clone()));
        return;
    }
        ...
        if (auto* objEnumType = dynamic_cast<ast::TypeName*>(it->second.get())) {
            if (objEnumType->identifier) {
                ...
                if (env == "Result" && !objEnumType->genericArgs.empty()) {
                    ...
                    setType(node,  pal ? std::shared_ptr<ast::TypeNode>(pal->clone()) : nullptr);
                    setType(node,  std::shared_ptr<ast::TypeNode>(pal->clone()));
                    return;
                }
            }
        }
    ...
    setType(node,  fieldType ? std::shared_ptr<ast::TypeNode>(fieldType->clone()) : nullptr);
    setType(node,  std::shared_ptr<ast::TypeNode>(fieldType->clone()));
...
void SemanticAnalyzer::visit(ast::AssignmentExpression* node) {
    ...
    if (derefLHS) {
        ...
        auto ptrTypeIt = expressionTypes.find(exprKey(derefLHS->pointer.get()));
        if (ptrTypeIt != expressionTypes.end() && ptrTypeIt->second) {
            // Save the original type (pointee type)
            auto origTypeIt = expressionTypes.find(exprKey(derefLHS));
            ...
            setType(derefLHS,  ptrTypeIt->second);
            setType(derefLHS,  std::shared_ptr<ast::TypeNode>(ptrTypeIt->second->clone()));
        }
    }
    ...
    if (derefLHS) {
        if (savedDerefType) {
            setType(derefLHS,  savedDerefType ? std::shared_ptr<ast::TypeNode>(savedDerefType->clone()) : nullptr);
            setType(derefLHS,  std::shared_ptr<ast::TypeNode>(savedDerefType->clone()));
        }
    }

    if (!isLValue(node->left.get())) {
        addError("LHS of assignment is not a valid L-value.", node->left.get());
        setType(node,  nullptr); // Mark as error
        return;
    }
    ...
    if (chk.code == IntAssignCode::NotInteger &&
        ...
        setType(node,  leftType ? std::shared_ptr<ast::TypeNode>(leftType->clone()) : nullptr); // Still use left type for further analysis
    } else {
        // The type of the assignment expression is typically the type of the LHS (or RHS after conversion).
        setType(node,  leftType ? std::shared_ptr<ast::TypeNode>(leftType->clone()) : nullptr); // Or rightType, depending on language rules for assignment expr type
    }

    if (leftType) {
      setType(node,  std::shared_ptr<ast::TypeNode>(leftType->clone()));
    }
...
void SemanticAnalyzer::visit(ast::LogicalExpression* node) {
    ...
    setType(node,  std::move(boolType));
}
...
void SemanticAnalyzer::visit(ast::ConditionalExpression* node) {
    ...
    auto conditionTypeIt = expressionTypes.find(exprKey(node->condition.get()));
    ...
    auto thenTypeIt = expressionTypes.find(exprKey(node->thenExpr.get()));
    auto elseTypeIt = expressionTypes.find(exprKey(node->elseExpr.get()));
    ...
    if (!areTypesCompatible(thenTypeIt->second.get(), elseTypeIt->second.get()) &&
        ...
        setType(node,  thenTypeIt->second);
        if (thenTypeIt->second) {
            setType(node,  std::shared_ptr<ast::TypeNode>(thenTypeIt->second->clone()));
        }
        return;
    }
...
void SemanticAnalyzer::visit(ast::SequenceExpression* node) {
    ...
    if (node->expressions.empty()) {
        ...
        setType(node,  emptyTupleType ? std::shared_ptr<ast::TypeNode>(emptyTupleType->clone()) : nullptr);
        setType(node,  std::shared_ptr<ast::TypeNode>(emptyTupleType.release()));
        return;
    }
    ...
    for (auto& expr : node->expressions) {
        if (expr) {
            expr->accept(*this);
            auto exprTypeIt = expressionTypes.find(exprKey(expr.get()));
    ...
    setType(node,  tupleType ? std::shared_ptr<ast::TypeNode>(tupleType->clone()) : nullptr);
    setType(node,  std::shared_ptr<ast::TypeNode>(tupleType.release()));
}
void SemanticAnalyzer::visit(ast::ObjectLiteral* node) {
    ...
    if (!typeName || !typeName->identifier) {
        ...
        setType(node,  retainType(node->typePath->clone().release()));
        return;
    }
    ...
    if (structFieldsIt == structFieldTypes.end()) {
        ...
        for (auto& prop : node->properties) {
            if (prop.key && prop.value) {
                auto valueTypeIt = expressionTypes.find(exprKey(prop.value.get()));
    ...
    if (!typeName->genericArgs.empty() && genericParamOrder &&
        ...
        setType(node,  retainType(node->typePath->clone().release()));
        setType(node,  std::shared_ptr<ast::TypeNode>(node->typePath->clone()));
        return;
    }
...
void SemanticAnalyzer::visit(ast::ArrayLiteral* node) {
    ...
    for (auto& element : node->elements) {
        if (element) {
            ...
            auto elemTypeIt = expressionTypes.find(exprKey(element.get()));
    ...
    if (elementType) {
        ...
        setType(node,  retainType(arrayType.release()));
...
void SemanticAnalyzer::visit(ast::FunctionExpression* node) {
    ...
    if (node->body) {
        if (auto* blockBody = dynamic_cast<ast::BlockExpression*>(node->body.get())) {
            ...
            scan = [&](const std::vector<vyb::ast::Statement*>& stmts) {
                for (vyb::ast::Statement* stmt : stmts) {
                    ...
                    if (auto* ret = dynamic_cast<ast::ReturnStatement*>(stmt)) {
                        if (ret->argument) {
                            auto ait = expressionTypes.find(exprKey(ret->argument.get()));
                            if (ait != expressionTypes.end() && ait->second) {
                                retTy = ait->second.get();
                            } else if (typeOf(ret->argument)) {
                                retTy = typeOf(ret->argument).get();
                            }
                        } else {
            ...
            auto it = expressionTypes.find(exprKey(node->body.get()));
            if (it != expressionTypes.end()) bodyTy = it->second.get();
            else if (typeOf(node->body)) bodyTy = typeOf(node->body).get();
    ...
    setType(node,  retainType(funcType));
    setType(node,  std::shared_ptr<ast::TypeNode>(funcType->clone()));
}
...
void SemanticAnalyzer::visit(ast::AwaitExpression* node) {
    ...
    auto it = expressionTypes.find(exprKey(node->expr.get()));
    if (it != expressionTypes.end()) operandTy = it->second.get();
    else if (typeOf(node->expr)) operandTy = typeOf(node->expr).get();
    ...
    if (inner) {
        setType(node,  std::shared_ptr<ast::TypeNode>(inner->clone()));
        setType(node,  typeOf(node) ? std::shared_ptr<ast::TypeNode>(typeOf(node)->clone()) : nullptr);
    } else {
        setType(node,  nullptr);
    }
                ...
                for (auto it = handlerBlock->body.rbegin(); it != handlerBlock->body.rend(); ++it) {
                    if (auto* exprStmt = dynamic_cast<ast::ExpressionStatement*>(it->get())) {
                        if (exprStmt->expression) {
                            auto eIt = expressionTypes.find(exprKey(exprStmt->expression.get()));
...
void SemanticAnalyzer::visit(ast::SelectExpression* node) {
    ...
    if (node->expr) {
        if (auto it = expressionTypes.find(exprKey(node->expr.get())); it != expressionTypes.end() && it->second) {
            selectTypeStr = it->second->toString();
        } else if (typeOf(node->expr)) {
            selectTypeStr = typeOf(node->expr)->toString();
        }
    }
                ...
                if (variantsIt != enumVariantPayloadTypes.end() && tv && tv->identifier) {
                    ...
                    if (varIt != variantsIt->second.end()) {
                        ...
                        for (size_t bi = 0; bi < ctor->arguments.size(); ++bi) {
                            ...
                            setType(bid,  pt ? std::shared_ptr<ast::TypeNode>(pt->clone().release()) : nullptr);
                        }
                    }
                }
            }
...
void SemanticAnalyzer::visit(ast::GenericInstantiationExpression* node) {
    ...
    if (auto identifier = dynamic_cast<ast::Identifier*>(node->baseExpression.get())) {
        if (enumGenericParamOrder.count(identifier->name)) {
            ...
            setType(node,  retainType(tn));
            setType(node,  std::shared_ptr<ast::TypeNode>(tn->clone()));
            return;
        }
...
void SemanticAnalyzer::visit(ast::PointerDerefExpression* node) {
    ...
    auto pointerTypeIt = expressionTypes.find(exprKey(node->pointer.get()));
    ...
    if (!pointeeType) {
        ...
        setType(node,  nullptr);
        if (typeOf(node)) typeOf(node).reset();
        return;
    }

    setType(node,  retainType(pointeeType)); // raw pointer, ownership handled by clone().release()
    setType(node,  std::shared_ptr<ast::TypeNode>(pointeeType->clone())); // typeOf(node) takes ownership of a new clone
}
void SemanticAnalyzer::visit(ast::AddrOfExpression* node) {
    ...
    auto operandTypeIt = expressionTypes.find(exprKey(node->getLocation().get()));
    ...
    setType(node,  retainType(addrAstType));
    setType(node,  std::shared_ptr<ast::TypeNode>(addrAstType->clone()));
}
void SemanticAnalyzer::visit(ast::FromIntToLocExpression* node) {
    ...
    auto addrExprTypeIt = expressionTypes.find(exprKey(node->getAddressExpression().get()));
    ...
    setType(node,  retainType(targetType->clone().release()));
    setType(node,  std::shared_ptr<ast::TypeNode>(targetType->clone()));
}
void SemanticAnalyzer::visit(ast::LocationExpression* node) {
    ...
    auto it = expressionTypes.find(exprKey(node->expression.get()));
    if (it == expressionTypes.end() || !it->second) {
    ...
    setType(node,  retainType(typeName));
    setType(node,  std::shared_ptr<ast::TypeNode>(typeName->clone()));
}
...
void SemanticAnalyzer::visit(ast::ReturnStatement* node) {
    ...
    if (node->argument) {
        ...
        if (currentFunction && currentFunction->returnTypeNode) {
            ast::TypeNode* retTy = typeOf(currentFunction->returnTypeNode)
                ? typeOf(currentFunction->returnTypeNode).get()
        ...
        if (currentFunction && currentFunction->returnTypeNode) {
            ast::TypeNode* retTy = typeOf(currentFunction->returnTypeNode)
                ? typeOf(currentFunction->returnTypeNode).get()
                : currentFunction->returnTypeNode.get();
            if (retTy) injectBareEnumConstructorType(*this, node->argument.get(), retTy);
        }
...
void SemanticAnalyzer::validateReturnArity(ast::ReturnStatement* node) {
    ...
    if (auto seq = dynamic_cast<ast::SequenceExpression*>(node->argument.get())) {
        ...
        auto it = expressionTypes.find(exprKey(node->argument.get()));
        if (it != expressionTypes.end() && it->second) {
            if (auto tt = dynamic_cast<ast::TupleTypeNode*>(it->second.get())) {
                actual = tt->memberTypes.size();
            }
        } else if (auto tt = dynamic_cast<ast::TupleTypeNode*>(typeOf(node->argument).get())) {
            actual = tt->memberTypes.size();
        }
    }
...
void SemanticAnalyzer::visit(ast::MatchStatement* node) {
    ...
    if (node->expr) {
        if (auto it = expressionTypes.find(exprKey(node->expr.get())); it != expressionTypes.end() && it->second) {
            ...
            matchTypeStr = typeOf(node->expr)->toString();
        }
    }
            ...
            if (!matchTypeStr.empty() && coreTypeName(matchTypeStr) != structName) {
                ...
                if (fieldsIt == structFieldTypes.end()) {
                    ...
                    for (auto& binding : sp->bindings) {
                        ...
                        setType(binding.get(),  fieldType ? std::shared_ptr<ast::TypeNode>(fieldType->clone()) : nullptr);
                    }
                }
            }
...
void SemanticAnalyzer::visit(ast::StructDeclaration* node) {
    ...
    auto structType = retainType(new ast::TypeName(node->loc, std::make_unique<ast::Identifier>(node->name->loc, structName)));
...
void SemanticAnalyzer::visit(ast::FailStatement* node) {
    ...
    if (node->errorType) {
        ...
        explicitErrorType = typeOf(node->errorType) ? typeOf(node->errorType).get() : node->errorType.get();
    }

    // Get the type of the error expression
    auto it = expressionTypes.find(exprKey(node->error.get()));
    if (it != expressionTypes.end() && it->second) {
...
void SemanticAnalyzer::visit(ast::RefailStatement* node) {
    ...
    if (node->wrappedError) {
        ...
        auto it = expressionTypes.find(exprKey(node->wrappedError.get()));
        if (it != expressionTypes.end() && it->second) {
...
void SemanticAnalyzer::visit(ast::PanicStatement* node) {
    ...
    auto it = expressionTypes.find(exprKey(node->message.get()));
    if (it != expressionTypes.end() && it->second) {
...
void SemanticAnalyzer::visit(ast::ExitStatement* node) {
    ...
    auto it = expressionTypes.find(exprKey(node->code.get()));
    if (it != expressionTypes.end() && it->second) {
...
void SemanticAnalyzer::visit(ast::TypeName* node) {
    ...
    if (typeNameStr == "Self") {
        if (currentImplType) {
            ...
            setType(node,  currentImplType ? std::shared_ptr<ast::TypeNode>(currentImplType->clone()) : nullptr);
            return;
        } else {
            ...
            setType(node,  std::shared_ptr<ast::TypeNode>(node->clone()));
            return;
        }
    }
    ...
    if (currentImplType && !currentImplTraitName.empty()) {
        ...
        if (!resolvedAssocType.empty()) {
            setType(node,  std::shared_ptr<ast::TypeNode>(
    ...
    if (typeNameStr == "Vec") {
        ...
        setType(node,  std::shared_ptr<ast::TypeNode>(vecType.release()));
        ...
        setType(node,  std::shared_ptr<ast::TypeNode>(futureType.release()));
        ...
        if (!node->genericArgs.empty()) {
            ...
            setType(node,  std::shared_ptr<ast::TypeNode>(symbol->type->clone()));
        }
    }
}
void SemanticAnalyzer::visit(ast::ConstructionExpression* node) {
    ...
    if (!typeNameNode || !typeNameNode->identifier) {
         ...
         setType(node,  retainType(node->constructedType->clone().release()));
         if (expressionTypes[exprKey(node)]) {
            setType(node,  std::shared_ptr<ast::TypeNode>(expressionTypes[exprKey(node)]->clone()));
         }
         for (auto& arg : node->arguments) {
             if (arg) arg->accept(*this);
    ...
    if (constructedName == "addr") {
        ...
        if (node->arguments.size() != 1 || !node->arguments[0]) {
            addError("addr() intrinsic expects 1 argument.", node); setType(node,  nullptr); return;
        }
        node->arguments[0]->accept(*this);
        ast::Expression* argExpr = node->arguments[0].get();
        if (!isLValue(argExpr)) {
            addError("Argument to addr() must be an L-value.", node); setType(node,  nullptr); return;
        }
        auto argTypeIt = expressionTypes.find(exprKey(argExpr));
        if (argTypeIt == expressionTypes.end() || !argTypeIt->second) {
            addError("Argument to addr() has an unknown type.", node); setType(node,  nullptr); return;
        }
        ...
        setType(node,  retainType(resType));
        setType(node,  std::shared_ptr<ast::TypeNode>(resType->clone()));
        ...
        if (typeNameNode->genericArgs.empty() || !typeNameNode->genericArgs[0]) {
            addError("from<TargetType>() intrinsic requires TargetType as a generic argument.", node); setType(node,  nullptr); return;
        }
        if (node->arguments.size() != 1 || !node->arguments[0]) {
            addError("from<T>() intrinsic expects 1 argument value.", node); setType(node,  nullptr); return;
        }
        ...
        if (!isTargetLocOrPointer) {
             addError("Target type in from<T>() must be a location/pointer type (e.g., loc<ActualType>).", node); setType(node,  nullptr); return;
        }
        node->arguments[0]->accept(*this);
        ast::Expression* addrValExpr = node->arguments[0].get();
        auto addrValTypeIt = expressionTypes.find(exprKey(addrValExpr));
        ...
        if (!isAddrValInteger) {
            ...
            setType(node,  nullptr); return;
        }
        setType(node,  retainType(targetTypeAst->clone().release()));
        setType(node,  std::shared_ptr<ast::TypeNode>(targetTypeAst->clone()));

    } else if (constructedName == "at") {
        // ... (at handling logic as before, ensure it's correct) ...
        if (node->arguments.size() != 1 || !node->arguments[0]) {
            addError("at() intrinsic expects 1 argument (the pointer).", node); setType(node,  nullptr); return;
        }
        node->arguments[0]->accept(*this);
        ast::Expression* ptrExpr = node->arguments[0].get();
        auto ptrTypeIt = expressionTypes.find(exprKey(ptrExpr));
        if (ptrTypeIt == expressionTypes.end() || !ptrTypeIt->second) {
            addError("Cannot dereference pointer with unknown type using at().", node); setType(node,  nullptr); return;
        }
        ...
        if (auto pt = dynamic_cast<ast::PointerType*>(resolvedPointerType)) {
            ...
            if (locTn->identifier->name == "loc" && !locTn->genericArgs.empty() && locTn->genericArgs[0]) {
                pointeeType = locTn->genericArgs[0]->clone().release();
                 ...
                 addError("Type for at() is \'loc\' but missing type parameter (e.g. loc<T>).", node); setType(node,  nullptr); return;
            }
        } else {
        ...
        if (!pointeeType) {
             addError("Failed to determine pointee type for at() operation.", node); setType(node,  nullptr); return;
        }
        setType(node,  retainType(pointeeType));
        setType(node,  std::shared_ptr<ast::TypeNode>(pointeeType->clone()));
    } else {
        node->constructedType->accept(*this);
        setType(node,  retainType(node->constructedType->clone().release()));
        if (expressionTypes[exprKey(node)]) {
...
void SemanticAnalyzer::visit(ast::BorrowExpression* node) {
    ...
    auto exprTypeIt = expressionTypes.find(exprKey(node->expression.get()));
...
void SemanticAnalyzer::handleTemplateInstantiation(ast::Identifier* identifier,
    ...
    if (instantiated) {
        ...
        setType(node,  nullptr); // Will be properly typed later
    }
}
...
bool SemanticAnalyzer::setResolvedTraitReturnType(ast::CallExpression* callNode,
    ...
    if (!resolvedAssocType.empty()) {
        ...
        setType(callNode,  retainType(resolvedType));
        setType(callNode,  std::shared_ptr<ast::TypeNode>(resolvedType->clone()));
        return true;
    }

    setType(callNode,  traitReturnType ? std::shared_ptr<ast::TypeNode>(traitReturnType->clone()) : nullptr);
    setType(callNode,  std::shared_ptr<ast::TypeNode>(traitReturnType->clone()));
    return true;
}
...
void SemanticAnalyzer::visit(ast::AsExpression* node) {
    ...
    ast::TypeNode* targetType = typeOf(node->targetType)
        ? typeOf(node->targetType).get() : node->targetType.get();
    if (!targetType) {
    ...
    ast::TypeNode* operandType = typeOf(node->operand)
        ? typeOf(node->operand).get() : nullptr;
    if (!operandType) {
        auto it = expressionTypes.find(exprKey(node->operand.get()));
...
void SemanticAnalyzer::visit(ast::TupleDestructureAssignment* node) {
    ...
    } else if (typeOf(node->expression)) {
        rhsType = typeOf(node->expression).get();
    }
