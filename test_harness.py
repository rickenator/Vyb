#!/usr/bin/env python3
"""
Vyn Modern Test Harness v2.0

A comprehensive test runner with advanced reporting, parallel execution,
and intelligent test discovery for the Vyn programming language.
...
@dataclass
class TestCase:
    """Represents a single Vyn test case with comprehensive metadata."""
    filename: str
...
class TestDiscovery:
    ...
    def __init__(self):
        self.vyn_root = self._find_vyn_root()
    
    def _find_vyn_root(self) -> Path:
    ...
    def discover_tests(self, test_dirs: List[str] = None, patterns: List[str] = None) -> List[TestCase]:
        ...
        for test_dir in test_dirs:
            test_path = self.vyn_root / test_dir
            ...
            for pattern in patterns:
                for file_path in test_path.rglob(pattern):
                    if file_path.is_file():
                        ...
                        if test_case:
                            # Use relative path from vyn_root for the filename
                            relative_path = file_path.relative_to(self.vyn_root)
...
class TestRunner:
    ...
    def __init__(self, vyn_executable: str, max_workers: int = None):
        self.vyn_executable = vyn_executable
    ...
    def _run_single_test(self, test: TestCase, verbose: bool = False) -> TestResult:
        ...
        try:
            ...
            process = subprocess.run(
                ...
                cwd=str(vyn_root)  # Run from vyn root directory
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Vyn Modern Test Harness v2.0',
...
Examples:
  ...
  ./test_harness.py --pattern "test_*_basic.vyn"
  ...
  ./test_harness.py --priority high --tags async,debug
    ...
    parser.add_argument('--vyn', default=None, help='Path to vyn executable')
    parser.add_argument('--test-dirs', nargs='+', default=['test'], help='Test directories')
    parser.add_argument('--pattern', nargs='+', default=['*.vyn'], help='File patterns')
    ...
    # Set default vyn executable
    if args.vyn is None:
        vyn_executable = str(discovery.vyn_root / "build" / "vyn")
    else:
        vyn_executable = args.vyn
    
    # Verify executable exists
    if not os.path.isfile(vyn_executable):
        print(f"{Colors.RED}Error: Cannot find Vyn executable at {vyn_executable}{Colors.RESET}")
        print(f"Build Vyn or specify executable with --vyn option")
        sys.exit(1)
    
    print(f"{Colors.BLUE}Vyn Test Harness v2.0{Colors.RESET}")
    print(f"Using executable: {vyn_executable}")
    ...
    # Run tests
    runner = TestRunner(vyn_executable, args.workers)
